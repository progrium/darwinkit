...
for framework in "${FRAMEWORKS[@]}"; do
    ...
    go run cmd/tools/fetch_apple_docs/main.go "$framework"
...
go run cmd/tools/docs_analyzer/main.go --outdir="$OUTPUT_DIR/analysis"

echo "Analyzing darwinkit coverage..."
# Run the coverage analyzer
go run cmd/tools/coverage_analyzer/main.go --apple-docs="$OUTPUT_DIR" --out="$OUTPUT_DIR/analysis/coverage_report.json"

echo "Generating coverage report..."
# Generate the coverage report
go run cmd/tools/report_generator/main.go --coverage="$OUTPUT_DIR/analysis/coverage_report.json" --output="API_COVERAGE.md"
